...
def process_image(image_path: str, zone_color: Tuple[int, int, int]) -> Tuple[np.ndarray, list]:
    ...
    print(f"Nombre de contours détectés pour la zone mauve : {len(zone_contours)}")
...
def is_point_near_border(x: int, y: int, contours: list, km_per_pixel: float, border_threshold: float = 0.5) -> Tuple[bool, str]:
    ...
    if contours:
        ...
        if min_distance <= border_threshold:
            near_border = True
            border_warning += (
                f"⚠️ ATTENTION : Le bateau est à {min_distance:.2f} km de la frontière de la zone interdite pour la pêche. "
...
def check_location(lat: float, lon: float, x: int, y: int, contours: list, km_per_pixel: float) -> str:
    ...
    if in_zone:
        status = (
            "🚨 ALERTE : Le bateau est dans une zone INTERDITE pour la pêche aux filets traînants (zone mauve). "
        ...
        status = (
            "✅ Le bateau est actuellement dans une zone autorisée pour la pêche aux filets traînants (hors zone mauve). "
...
def place_point_deg_min(lat_deg_min: str, lon_deg_min: str, point_size: int = 5, point_color: str = "red"):
    ...
    IMAGE_PATH = "carte/carte_tunisie_mauve.PNG"
    ZONE_COLOR = (128, 0, 128)  # Mauve (approximation RGB pour les zones interdites)
    MARGIN_LEFT = 50
    MARGIN_RIGHT = 50
    MARGIN_TOP = 50
    MARGIN_BOTTOM = 50
    ...
    lat_min_map = deg_min_to_decimal(32, 40)
...
if __name__ == "__main__":
    x, y = place_point_deg_min("35°00", "12°00")