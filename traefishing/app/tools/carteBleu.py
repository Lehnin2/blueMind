...
def process_image(image_path: str, zone_color: Tuple[int, int, int]) -> Tuple[np.ndarray, list]:
    ...
    print(f"Nombre de contours détectés pour la zone bleu clair : {len(zone_contours)}")
...
def is_point_near_border(x: int, y: int, contours: list, km_per_pixel: float, border_threshold: float = 0.5) -> Tuple[bool, str]:
    ...
    if contours:
        ...
        if min_distance <= border_threshold:
            near_border = True
            border_warning += (
                f"⚠️ ATTENTION : Le bateau est à {min_distance:.2f} km de la frontière de la zone interdite pour la pêche aux filets tournants. "
...
def check_location(lat: float, lon: float, x: int, y: int, contours: list, km_per_pixel: float) -> str:
    ...
    if in_zone:
        status = (
            "🚨 ALERTE : Le bateau est dans une zone INTERDITE pour la pêche aux filets tournants  (zone bleu clair). "
        ...
        status = (
            "✅ Le bateau est actuellement dans une zone autorisée pour la pêche aux filets tournants  (hors zone bleu clair). "
...
def place_point_deg_min(lat_deg_min: str, lon_deg_min: str, point_size: int = 5, point_color: str = "red"):
    ...
    IMAGE_PATH = "carte/carte_tunisie_bleu.PNG"  # Ajuste ce chemin si nécessaire
    ZONE_COLOR = (135, 206, 250)  # Bleu clair (approximation RGB pour les zones interdites)
    MARGIN_LEFT = 50
    MARGIN_RIGHT = 50
    MARGIN_TOP = 50
    MARGIN_BOTTOM = 50
    ...
    lat_max_map = deg_min_to_decimal(37, 48)
