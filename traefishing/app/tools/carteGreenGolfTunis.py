...
def process_image(image_path: str, zone_color: Tuple[int, int, int]) -> Tuple[np.ndarray, list]:
    ...
    print(f"Nombre de contours détectés pour la zone : {len(zone_contours)}")
...
def is_point_near_border(x: int, y: int, contours: list, km_per_pixel: float, border_threshold: float = 0.5) -> Tuple[bool, str]:
    ...
    if contours:
        ...
        if min_distance <= border_threshold:
            near_border = True
            border_warning += (
                f"⚠️ ATTENTION : Le bateau est à {min_distance:.2f} km de la frontière de la zone autorisée pour la pêche. "
                "Risque de sortie imminente de la zone réglementée ! "
                "Veuillez ajuster votre trajectoire pour rester dans la zone autorisée."
            )
...
def check_location(lat: float, lon: float, x: int, y: int, contours: list, km_per_pixel: float) -> str:
    ...
    if in_zone:
        status = (
            "✅ Le bateau est actuellement dans la zone autorisée pour la pêche aux filets traînants dans le Golfe de Tunis. "
            "Poursuivez vos activités en toute conformité avec la réglementation en vigueur."
        )
    else:
        status = (
            "🚨 ALERTE : Le bateau est HORS de la zone autorisée pour la pêche aux filets traînants dans le Golfe de Tunis. "
            "Ceci constitue une violation des réglementations de pêche pour le mois de juillet. "
            "Veuillez immédiatement repositionner votre bateau dans la zone autorisée sur la carte."
        )
...
def place_point_deg_min(lat_deg_min: str, lon_deg_min: str, point_size: int = 5, point_color: str = "red"):
    ...
    IMAGE_PATH = "carte/carte_true_gulf_tunis.PNG"
    ZONE_COLOR = (0, 255, 0)  # Vert pour la zone autorisée
    MARGIN_LEFT = 50
    MARGIN_RIGHT = 50
    MARGIN_TOP = 50
    MARGIN_BOTTOM = 50
    ...
    # Limites de la carte
    lon_min_map = deg_min_to_decimal(10, 0)
    lon_max_map = deg_min_to_decimal(11, 30)
    lat_min_map = deg_min_to_decimal(36, 30)
    lat_max_map = deg_min_to_decimal(37, 30)
...
if __name__ == "__main__":
    x, y = place_point_deg_min("37°00", "11°00")